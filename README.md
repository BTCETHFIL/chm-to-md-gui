...
│  [7-Zip] 正在提取...                        ✓ 完成     │
│  完成: ABCONTROLLOGIX_Chs.chm → 156 个 .md             │
│  处理: BACnetIP_Chs.chm                                │
│  ────────────────────────────────────                  │
│                                                        │
│  总计: 7xxx 个 .md 文件（来自 46 个 CHM）               │
└──────────────────────────────────────────────────────┘
...
> 只包含转换后的 `.md` 文件，按 TOC 层级组织，目录名和文件名都带数字编号。无多余文件。
...
> 目录名格式：`序号_驱动名称`（如 `12_InovanceTcpNet驱动`）。<br>
> 无冗余包裹层，所有内容直接放在根目录下，清晰简洁。<br>
